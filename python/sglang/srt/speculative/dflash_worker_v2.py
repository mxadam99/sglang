    ...
    def forward_batch_generation(
        ...
        if draft_bs:
            ...
            if folded:
                ...
                if self._uses_confidence_scheduling():
                    self._selector_confidence = selector_selected_path_confidence(
                        self._draft_sampler.scores_out[:draft_bs],
                        self._draft_sampler.path_indices_out[:draft_bs],
                    )
        ...
        if confidence_layout is None:
            ...
            effective_verify_budget = (
                block_size if self._uses_confidence_scheduling() else verify_budget
            )
            verify_input_ids = draft_tokens[:, :effective_verify_budget].reshape(-1)
            verify_positions = positions_2d[:, :effective_verify_budget].reshape(-1)
            batch.out_cache_loc = verify_out_cache_loc_2d[
                :, :effective_verify_budget
            ].reshape(-1)
            runner = self.model_runner.decode_cuda_graph_runner
            graph_buckets = self._ragged_verify_graph_buckets()
            if (
                ...
                and getattr(runner, "ragged_verify_mode", False)
                and graph_buckets
                and bs * effective_verify_budget <= graph_buckets[-1]
            ):
                ...
                confidence_layout = RaggedVerifyLayout.from_verify_lens(
                    verify_lens_cpu=[effective_verify_budget] * bs,
                    device=device,
                    grid=graph_buckets,
                )
        ...
        verify_tokens = draft_tokens[:, :candidate_width]
        grammar_tree = (
        ...
        if seq_lens_cpu_backup is not None:
            ...
            verify_host_seq_lens = seq_lens_cpu_backup + (
                block_size if self._uses_confidence_scheduling() else verify_budget
            )
        ...
        if sampling_info is not None:
            apply_dflash_verify_logits_adjustments(
                ...
                sampling_info=sampling_info,
                draft_token_num=candidate_width,
            )
        ...
        if not folded_accept:
            ...
            hidden = hidden.view(bs, candidate_width, -1)

            self._append_target_hidden_to_draft_kv_by_loc(
                target_hidden=hidden.reshape(-1, hidden.shape[-1]),
                cache_loc=verify_out_cache_loc_2d[:, :candidate_width].reshape(-1),
                cache_loc_2d=verify_out_cache_loc_2d[:, :candidate_width],
                positions=positions_2d[:, :candidate_width].reshape(-1),
                commit_lens=commit_lens,
            )
