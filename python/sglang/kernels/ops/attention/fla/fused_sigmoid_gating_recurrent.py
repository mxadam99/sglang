...
def fused_sigmoid_gating_delta_rule_recover_final_state_kernel(
    ...
    """Recover h_K without materializing intermediate states or outputs."""
    i_k, i_v, i_nh = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    i_n, i_hv = i_nh // HV, i_nh % HV
    i_h = i_hv // (HV // H)
    ...
    b_A_coeff = -tl.exp(tl.load(p_A_log).to(tl.float32))
    if IS_KDA:
        b_dt_bias = tl.load(p_dt_bias, mask=mask_k, other=0).to(tl.float32)
    else:
        b_dt_bias = tl.load(p_dt_bias).to(tl.float32)
...
def fused_sigmoid_gating_delta_rule_recover_final_state(
    ...
    """Recompute and commit only the accepted final GDN state."""
